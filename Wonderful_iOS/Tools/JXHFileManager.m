...
//  JXHApplication
//
//  Created by 郭玉泽 on 2019/1/17.
//  Copyright © 2019年 郭玉泽. All rights reserved.
//https://github.com/guodongyangwen/DYFileManager

#import "JXHFileManager.h"
#define FileManager     [NSFileManager defaultManager]
static BOOL enableLog = NO;

@implementation JXHFileManager

+ (id)defalutFileManager {
    static dispatch_once_t onceToken;
    static JXHFileManager* shareInstance = nil;
    dispatch_once(&onceToken, ^{
        shareInstance = [[JXHFileManager alloc]init];
    });
    return shareInstance;
}

+ (NSString *)webResource
{
    return [[self documentDirPath]stringByAppendingString:@"/webResource/"];
}
...
+ (BOOL)createFileWithPath:(NSString *)path content:(id)content overwrite:(BOOL)overwrite attributes:(NSDictionary *)attributes error:(NSError **)error {
    ...
    if (![self fileExistsAtPath:path]) {
        if([FileManager createFileAtPath:path contents:nil attributes:attributes]) {
            ...
            [JXHFileManager DYLog:@"创建文件异常，请检查文件路径"];
            return NO;
        }
    }
    else{
...
+ (BOOL)createDirectoryWithDirPath:(NSString *)path withIntermediateDirectories:(BOOL)IntermediateDirectories attributes:(NSDictionary*)attributes error:(NSError **)error {
    ...
    if (isExit) {
        if(isDirectory){
            [JXHFileManager DYLog:@"directory already exitst"];
            return YES;
        }
        else{
            [JXHFileManager DYLog:@"path format is wrong"];
            return NO;
        }
    }
    else{
...
+ (CGFloat)deviceFreeSpace
{
    NSDictionary *fattributes = [[NSFileManager defaultManager] attributesOfFileSystemForPath:NSHomeDirectory() error:nil];
    NSNumber *freeNum =  [fattributes objectForKey:NSFileSystemFreeSize];
    XXLog(@"value === %f MB",freeNum.floatValue/1024.0/1024.0);
    return freeNum.floatValue;
...
//
//        XXLog(@" totalsize %f G,freesize %f G",totalsize/1024.0/1024.0,freesize/1024.0/1024.0);
//    } else
//    {
//        XXLog(@"Error Obtaining System Memory Info: Domain = %@, Code = %ld", [error domain], (long)[error code]);
//    }
